...
def check_attention_mode(kind: str) -> None:
...
class AttentionDropoutTest(unittest.TestCase):
    """Cover every functional attention call in train and evaluation modes."""

    def test_attention_modes(self) -> None:
        """All attention implementations respect the module training flag."""
        for kind in ('self', 'forward', 'backward'):
            with self.subTest(kind=kind):
                check_attention_mode(kind)
